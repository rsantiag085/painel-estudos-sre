...
def prepare(session, start="2026-08-05", end="2026-08-09"):
...
def test_activity_prerequisite_must_be_done(session):
    ...
    with pytest.raises(SchedulingConflict, match="Pré-requisitos"):
        allocate_activity(session, activity.id, "2026-08-05-F1")

    session.get(ActivityProgress, "linux-admin-001").status = "done"
    session.flush()
    progress = allocate_activity(session, activity.id, "2026-08-05-F1")
    assert progress.current_slot_id == "2026-08-05-F1"
...
def test_course_prerequisite_must_be_completed(session):
    ...
    with pytest.raises(SchedulingConflict, match="Pré-requisitos"):
        allocate_activity(session, network_activity, "2026-08-05-F1")
    ...
    progress = allocate_activity(session, network_activity, "2026-08-05-F1")
    assert progress.current_slot_id == "2026-08-05-F1"
...
def test_duplicate_allocation_is_rejected_and_history_is_not_duplicated(session):
    ...
    with pytest.raises(SchedulingConflict, match="já possui slot"):
        allocate_activity(session, "linux-admin-001", "2026-08-05-F4")
    with pytest.raises(SchedulingConflict, match="não está disponível"):
        allocate_next(session, "2026-08-05-F3")
...
def test_in_progress_keeps_its_slot_while_other_slots_are_allocated(session):
    ...
    allocate_available_slots(session, date(2026, 8, 5), date(2026, 8, 6))

    assert progress.status == "in_progress"
    assert progress.current_slot_id == "2026-08-05-F3"
    assert session.get(StudySlot, "2026-08-05-F3").status == "scheduled"
...
def test_not_done_becomes_deferred_and_moves_to_next_compatible_slot(session):
    ...
    assert progress.current_slot_id == "2026-08-05-F4"
    assert session.get(StudySlot, "2026-08-05-F3").status == "missed"
    assert session.get(StudySlot, "2026-08-05-F4").status == "scheduled"
    deferred_event = session.scalars(
        select(ActivityHistory).where(ActivityHistory.event_type == "deferred")
    ).one()
    assert deferred_event.study_slot_id == "2026-08-05-F3"
...
def test_defer_expands_schedule_when_materialized_window_is_full(session):
    ...
    assert progress.current_slot_id == "2026-08-21-F2"
    assert session.get(StudySlot, "2026-08-21-F2").status == "scheduled"
...
def test_inactive_statuses_are_allowed_release_slot_and_leave_queue(session, status):
    ...
    assert session.get(StudySlot, "2026-08-05-F3").status == "available"
    ...
    assert event.study_slot_id == "2026-08-05-F3"


def test_flexible_slot_compatibility_for_aws_and_reading(session):
    prepare(session)
    activity = session.get(Activity, "linux-admin-001")
    f4 = session.get(StudySlot, "2026-08-05-F4")
    t2 = session.get(StudySlot, "2026-08-06-T2")
    f1 = session.get(StudySlot, "2026-08-05-F1")
